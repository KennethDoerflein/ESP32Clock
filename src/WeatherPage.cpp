...
void WeatherPage::update()
{
  ...
  if (current.isValid != _lastWeatherData.isValid || (current.isValid && (abs(current.temp - _lastWeatherData.temp) > 0.1 || strcmp(current.condition, _lastWeatherData.condition) != 0)))
  {
    _needsRedraw = true;
  }
